...
$sql = "SELECT a.*, u.NomComplet, 
        CASE 
            WHEN a.NumLotProd IS NOT NULL THEN 'Produit Fini'
            WHEN a.NumLotMP IS NOT NULL THEN 'Matière Première'
        END as TypeAlerte,
        CASE 
            WHEN a.NumLotProd IS NOT NULL THEN p.NomProd
            WHEN a.NumLotMP IS NOT NULL THEN m.NomMP
        END as NomProduit,
        a.dateAlerte as DateAlerte,
        a.description as Description
        FROM alerte a
        LEFT JOIN users u ON a.IdUser = u.IdUser
        LEFT JOIN lotproduitfini lpf ON a.NumLotProd = lpf.NumLotProd
        LEFT JOIN lotmatierepremiere lmp ON a.NumLotMP = lmp.NumLotMP
        LEFT JOIN produitfini p ON lpf.IdProd = p.IdProd
        LEFT JOIN matierepremiere m ON lmp.IdMP = m.IdMP
        LEFT JOIN lignelivraison ll ON ll.NumLotProd = lpf.NumLotProd
        LEFT JOIN bonlivraison bl ON bl.NumBonLiv = ll.NumBonLiv
        WHERE 
            (a.IdUser = ?)
            OR (
                u.role = 'fabricant'
                AND a.NumLotProd IS NOT NULL
                AND lpf.Etat = 'rappelé'
                AND bl.IdClient = ?
            )
        GROUP BY a.idAlerte
...
$stmt->execute([$_SESSION['iduser'], $_SESSION['iduser']]);
$result = $stmt;
?>
