  ...
  <main>
    ...
    <form method="get" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
     ...
     <button class="search-icon-bottun"><i class="fas fa-search"></i></button>
            ...
            <h1>Consulter les controles qualité des lots produits</h1>
...
try {
  ...
  $sql = "
    SELECT 
      ...
      lp.NumLotProd,
      p.nomProd
    FROM ResultatControleProd rpf
    JOIN OperationControle oc ON oc.NumOpCtrl = rpf.NumOpCtrl
    JOIN Users u ON u.Iduser = oc.IdResCtrl
    JOIN LotProduitFini lp ON lp.NumLotProd = rpf.NumLotProd
  ...
  if ($query !== '') {
    $sql .= " WHERE rpf.NumLotProd = :query";
    $params = [':query' => $query];
  }
  ...
  if ($results) {
    ...
    if ($query !== '') {
      echo "<tr><td colspan='8'>Aucun résultat trouvé pour ce lot produit fini.</td></tr>";
    }
  }

} catch (PDOException $e) {
  echo "<tr><td colspan='8'>Erreur de connexion : " . $e->getMessage() . "</td></tr>";
}
?>
